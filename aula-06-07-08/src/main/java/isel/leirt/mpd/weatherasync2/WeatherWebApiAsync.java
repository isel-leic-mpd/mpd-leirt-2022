...
import static isel.leirt.mpd.weatherasync2.NioUtils.*;
...
public class WeatherWebApiAsync {
    ...
    /**
     * Get WeatherInfo from a local coordinates
     * @param lat
     * @param lon
     * @return
     */
    ...
    private CompletableFuture<AsynchronousSocketChannel>
        serverConnectionCF() {
    ...
    public CompletableFuture<String>
           weatherAtCF(double lat, double lon) {
        String request =
            "GET " + String.format(WEATHER_AT_TEMPLATE, lat, lon, API_KEY) + HEADERS;
        ...
        return
            serverConnectionCF()
            .thenCompose(channel ->
                writeCF(channel, request)
                .thenCompose(__ -> readCF(channel))
